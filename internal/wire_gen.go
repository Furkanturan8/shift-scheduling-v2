...
import (
	...
	"shift-scheduling-V2/docs"
	"shift-scheduling-V2/internal/api"
	"shift-scheduling-V2/internal/app"
	"shift-scheduling-V2/internal/common/errors"
	"shift-scheduling-V2/internal/common/logger"
	"shift-scheduling-V2/internal/infrastructure/cache"
	"shift-scheduling-V2/internal/infrastructure/notification"
	"shift-scheduling-V2/internal/infrastructure/persistence"
	"shift-scheduling-V2/internal/middleware/fibercasbin"
	"shift-scheduling-V2/internal/probes"
...
func New() (*Server, error) {
	...
	cragRepository := persistence.NewCragMemRepository()
	userRepository := persistence.NewUserMemRepository()
	loggerLogger := logger.NewLoggerAplication(configuration)
	service := notification.NewNotificationService(loggerLogger)
	application := app.NewApplication(cragRepository, userRepository, service, loggerLogger)
	cragHttpApi := api.NewCragHttpApi(application)
	cragRouter := router.NewCragRouter(cragHttpApi)
	userHttpApi := api.NewUserHttpApi(application)
	...
	server := NewServer(configuration, cragRouter, userRouter, healthCheckApplication, loggerLogger, engine)
	return server, nil
}
